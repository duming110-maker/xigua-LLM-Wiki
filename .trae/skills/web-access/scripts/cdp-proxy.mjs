...
// --- human-pace 发布模式（反风控） ---
// 命中 ?pace=human 的写操作成功后，随机睡 [MIN, MAX] ms 再返回，模拟"操作一个停顿一个"。
// 默认 3000~8000 ms 与 spec §3.6 F6 推荐区间一致；通过 env 覆盖调参。
const HUMAN_PACE_MIN = parseInt(process.env.CDP_HUMAN_PACE_MIN || '3000');
const HUMAN_PACE_MAX = parseInt(process.env.CDP_HUMAN_PACE_MAX || '8000');
function humanSleep() {
  // 在 [MIN, MAX] 之间取一个随机毫秒数，等待后 resolve。MIN>MAX 时退化为 MIN。
  const lo = Math.min(HUMAN_PACE_MIN, HUMAN_PACE_MAX);
  const hi = Math.max(HUMAN_PACE_MIN, HUMAN_PACE_MAX);
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    // GET /cookies?domain=xxx - 浏览器级 Network.getCookies（无需 sessionId）
    // 关键价值：能拿到 HttpOnly / Secure cookie，这是页面内 document.cookie 永远拿不到的。
    // 用途：material-collector cookie 失效时，由 web-access 打开站点拿到登录态 cookie 写回 .cookies/。
    if (pathname === '/cookies') {
      const domain = (q.domain || '').trim();
      if (!domain) {
        res.statusCode = 400;
        res.end(JSON.stringify({ error: '需要 domain 参数，例如 /cookies?domain=www.example.com' }));
        return;
      }
      // Network.getCookies 的 urls 参数会让浏览器返回匹配这些 URL（含 host/session cookie）的 cookie。
      // 同时给 https 和 http，避免漏掉 Secure 仅 https 写入、或 http-only 站点。
      const resp = await sendCDP('Network.getCookies', {
        urls: [`https://${domain}`, `http://${domain}`],
      });
      // 浏览器可能返回同根域下所有子域的 cookie，按 cookie.domain 包含请求域过滤，避免噪音。
      const all = resp.result?.cookies || [];
      const filtered = all.filter((c) => {
        const d = (c.domain || '').toLowerCase();
        const want = domain.toLowerCase();
        // cookie.domain 可能是 ".example.com"（前导点表示含子域）或 "www.example.com"。
        return d === want || d === `.${want}` || want.endsWith(d) || d.endsWith(`.${want}`);
      });
      res.end(JSON.stringify({ domain, cookies: filtered }));
      return;
    }
    ...
    // POST /navigate?target=xxx (body=URL) - 导航（自动等待加载）
      ...
      // human-pace：导航是高风险动作，命中后睡随机 3-8s 再返回
      if (q.pace === 'human') await humanSleep();
    ...
    // POST /eval?target=xxx - 执行 JS
      ...
      if (resp.result?.result?.value !== undefined) {
        // human-pace：eval 常用于填表/提交等发布动作，命中后睡随机 3-8s
        if (q.pace === 'human') await humanSleep();
    ...
    // POST /click?target=xxx — JS 层面点击（简单快速，覆盖大多数场景）
      ...
      if (resp.result?.result?.value) {
        ...
        if (val.error) {
          ...
          // human-pace：点击成功后才停顿（失败不停顿，让 Agent 尽快看到错误）
          if (q.pace === 'human') await humanSleep();
          res.end(JSON.stringify(val));
        }
      } else {
    ...
    // POST /clickAt?target=xxx — CDP 浏览器级真实鼠标点击（算用户手势，能触发文件对话框、绕过反自动化检测）
      ...
      // human-pace：真实鼠标点击更接近发布动作（点发布按钮、点确认），命中后睡随机 3-8s
      if (q.pace === 'human') await humanSleep();
    ...
    // body: JSON { "selector": "input[type=file]", "files": ["/path/to/file1.png", "/path/to/file2.png"] }
      ...
      // human-pace：上传文件成功后睡随机 3-8s（等待页面预览/校验完成，模拟用户看一眼再下一步）
      if (q.pace === 'human') await humanSleep();
    ...
    // GET /scroll?target=xxx&y=3000 - 滚动
      ...
      // human-pace：滚动成功后睡随机 3-8s（模拟人在页面停留观看，不是机械连续滚）
      if (q.pace === 'human') await humanSleep();
      res.end(JSON.stringify({ value: resp.result?.result?.value }));
    }
    ...
    // GET /info?target=xxx - 获取页面信息
      ...
      res.end(JSON.stringify({
        ...
        endpoints: {
          '/health': 'GET - 健康检查',
          '/cookies?domain=': 'GET - 取浏览器 cookie（含 HttpOnly，document.cookie 拿不到）',
          '/targets': 'GET - 列出所有页面 tab',
          '/new': 'POST body=URL - 创建新后台 tab（自动等待加载）',
          '/close?target=': 'GET - 关闭 tab',
          '/navigate?target=&pace=human': 'POST body=URL - 导航（自动等待加载）',
          '/back?target=': 'GET - 后退',
          '/info?target=': 'GET - 页面标题/URL/状态',
          '/eval?target=&pace=human': 'POST body=JS表达式 - 执行 JS',
          '/click?target=&pace=human': 'POST body=CSS选择器 - 点击元素',
          '/clickAt?target=&pace=human': 'POST body=CSS选择器 - 真实鼠标点击',
          '/setFiles?target=&pace=human': 'POST body=JSON - 文件上传',
          '/scroll?target=&y=&direction=&pace=human': 'GET - 滚动页面',
