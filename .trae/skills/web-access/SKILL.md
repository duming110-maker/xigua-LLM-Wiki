...
description:
  联网操作 skill，处理需要真实浏览器环境或登录态的网络任务：搜索、网页抓取、登录后操作、动态渲染页面、社交媒体抓取（小红书/微博/推特等）。
  触发场景：用户要求搜索信息、查看网页内容、访问需要登录的网站、操作网页界面、抓取社交媒体内容、读取动态渲染页面。
  注：纯下载/ASR 的素材抓取由 material-collector 自行处理，不强制走本 skill。
metadata:
  author: 一泽Eze
  version: "2.5.3"
---
...
# 取浏览器 cookie（含 HttpOnly，document.cookie 拿不到）——用于 cookie 失效刷新
...
**用途**：发布内容时模拟人工节奏——「操作一个停顿一个」，不一次性粘贴标题+简介+标签，避免被风控识别为机器批量提交。给写操作端点加 `?pace=human` 即启用，proxy 在操作成功后随机睡 3-8 秒（默认区间，env `CDP_HUMAN_PACE_MIN/MAX` 可调）再返回。

支持 `?pace=human` 的端点：`/navigate`、`/click`、`/clickAt`、`/eval`、`/scroll`、`/setFiles`。失败时不停顿（让 Agent 立即看到错误）。

```bash
# 发布时模拟人工节奏：粘贴标题 → 等 → 粘贴简介 → 等 → 点发布
curl -s -X POST "http://localhost:3456/eval?target=ID&pace=human" -d 'document.querySelector("input.title").value = "我的标题";'
curl -s -X POST "http://localhost:3456/eval?target=ID&pace=human" -d 'document.querySelector("textarea.intro").value = "我的简介";'
curl -s -X POST "http://localhost:3456/clickAt?target=ID&pace=human" -d 'button.publish'
```
